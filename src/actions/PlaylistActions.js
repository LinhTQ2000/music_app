...
export function loadUserPlaylists(constant) {
  return async (dispatch) => {
    ...
    try {
      dispatch(pending());
      const userPlaylists = await spotifyQuery("getUserPlaylists", [
...
async function getPlaylist(playlistId) {
  ...
  const areFollowingPlaylist = await spotifyQuery("areFollowingPlaylist", [
    playlistId,
    [userId],
  ]);
  return {
...
async function getPlaylistTracks(playlistId) {
  const response = await spotifyQuery("getPlaylistTracks", [
    playlistId,
  ...
  let mySavedTracks = [];
  if (trackIds.length) {
    mySavedTracks = await spotifyQuery("containsMySavedTracks", [trackIds]);
  }
...
export function loadRandomCategoryPlaylists() {
  return async (dispatch) => {
    ...
    try {
      ...
      const categoryPlaylists = await spotifyQuery("getCategoryPlaylists", [
        categoryId,
...
export function loadCategoryPlaylists(category = "") {
  return async (dispatch) => {
    ...
    try {
      dispatch(pending());
      const categoryById = await spotifyQuery("getCategory", [category.id]);
...
export function loadMoreCategoryPlaylists(id, offset) {
  return async (dispatch) => {
    ...
    try {
      ...
      const categoryPlaylists = await spotifyQuery("getCategoryPlaylists", [
        id,
        { offset, limit: CATEGORY_PLAYLISTS_LIMIT },
      ]);
      dispatch(
        success({
          items: transformResponse.playlists(categoryPlaylists.playlists.items),
...
export function createPlaylist(playlistName, redirect) {
  return async (dispatch) => {
    ...
    try {
      const userId = localStorage.getItem(USER_ID);
      const playlist = await spotifyQuery("createPlaylist", [
        userId,
        { name: playlistName },
      ]);
...
export function uploadCoverImage(playlistId, imageData) {
  return async (dispatch) => {
    ...
    try {
      await spotifyQuery("uploadCustomPlaylistCoverImage", [
        playlistId,
        imageData,
      ]);
      dispatch(success({ playlistId, image: imageData }));
    } catch (e) {
      console.error(e);
      alert.show(MESSAGES.ERROR);
    }
...
export function changePlaylistDetails(playlistId, data) {
  return async (dispatch) => {
    ...
    try {
      ...
      await spotifyQuery("changePlaylistDetails", [playlistId, data]);
      dispatch(success({ playlistId, key, value: data[key] }));
    } catch (e) {
      console.error(e);
      alert.show(MESSAGES.ERROR);
    }
