...
export function loadArtistTopTracks(artistId) {
  return async (dispatch) => {
    ...
    try {
      dispatch(pending());
      const response = await spotifyQuery("getArtistTopTracks", [
        artistId,
        "US",
      ]);
      const tracks = response.tracks;
      const trackIds = tracks.map((track) => track.id);
      const savedTracks = await spotifyQuery("containsMySavedTracks", [
        trackIds,
      ]);
      dispatch(
        success({
...
export function loadMySavedTracks() {
  return async (dispatch) => {
    ...
    try {
      dispatch(pending());
      const response = await spotifyQuery("getMySavedTracks", [
...
export function loadMorePlaylistTracks(playlistId, offset) {
  return async (dispatch) => {
    ...
    try {
      ...
      const response = await spotifyQuery("getPlaylistTracks", [
        playlistId,
        { limit: PLAYLIST_TRACKS_LIMIT, offset },
      ]);
      const tracks = response.items;
      const trackIds = tracks.map((item) => item.track.id);
      const savedTracks = await spotifyQuery("containsMySavedTracks", [
