...
export function categoryPlaylists(
  state = initialCategoryPlaylistsState,
  action
) {
  switch (action.type) {
...
export function featuredPlaylists(
  state = initialFeaturedPlaylistsState,
  action
) {
  switch (action.type) {
        ...
        items: state.items.map((item, index) => {
          if (item.id === action.payload) {
            ...
            return {
              ...state.items[index],
              meta:
                itemCount === 1 ? `${itemCount} track` : `${itemCount} tracks`,
            };
          }
          return item;
        }),
...
export function listUserPlaylists(
  state = initialListUserPlaylistsState,
  action
) {
  switch (action.type) {
    ...
    case PLAYLIST_TRACKS.LOAD_MORE_SUCCESS:
      return {
        ...
        isAvailableForPreview:
          state.isAvailableForPreview || action.payload.isAvailableForPreview,
      };
